...
//flag to initialize the display
boolean pixie_init = true;

//port 4 data
byte old_port4value = 0;
...
//pixie video flag to clear display
volatile boolean pixie_clear = true;
 ...
 *  
 *  E      6        13 SCK
 *                  12 MISO (Unused by display, used for Video Reset button)
 *  R/W    5        11 MOSI
 *  RS     4        10 CS
 *  
 *  VCC    2        +5V
...
void drawDigit(byte digit,int x) {
   static const uint16_t leds[16] = {
      0x0BBA,
      0x0529,
      0x1957,
      0x194E,
   ...
   for( int i=0; i<13; ++i ) {
      if( leds[digit] & mask ) {
         switch( i ) {
            case 0:
               u8g2.drawBox(x,4,8,8);
               break;
            case 1:
               u8g2.drawBox(x+8,4,8,8);
               u8g2.drawBox(x+16,4,8,8);
               break;
            case 2:
               u8g2.drawBox(x+24,4,8,8);
               break;

            case 3:
               u8g2.drawBox(x,12,8,8);
               u8g2.drawBox(x,20,8,8);
               break;
            case 4:
               u8g2.drawBox(x+24,12,8,8);
               u8g2.drawBox(x+24,20,8,8);
               break;

            case 5:
               u8g2.drawBox(x,28,8,8);
               break;
            case 6:
               u8g2.drawBox(x+8,28,8,8);
               u8g2.drawBox(x+16,28,8,8);
               break;
            case 7:
               u8g2.drawBox(x+24,28,8,8);
               break;
               
            case 8:
               u8g2.drawBox(x,36,8,8);
               u8g2.drawBox(x,44,8,8);
               break;
            case 9:
               u8g2.drawBox(x+24,36,8,8);
               u8g2.drawBox(x+24,44,8,8);
               break;
               
            case 10:
               u8g2.drawBox(x,52,8,8);
               break;
            case 11:
               u8g2.drawBox(x+8,52,8,8);
               u8g2.drawBox(x+16,52,8,8);
               break;
            case 12:
               u8g2.drawBox(x+24,52,8,8);
               break;
         } //switch
      } //if leds[digit] & mask
      mask >>= 1;
...
//Start the video
void startVideo(boolean display_init) {
  video_on = true;
  ...
  //stop watching port 4
   detachInterrupt(PORT_4_PIN);
  //start watching TPB
  attachInterrupt(TPB_PIN, isrTPB, RISING);  
} //startVideo

//Stop the video
void stopVideo(boolean display_clear) {
  //Stop watching TPB
  detachInterrupt(TPB_PIN);  
  //Start watching Port4
  attachInterrupt(PORT_4_PIN, isrPort4Output, FALLING);
  video_on = false;
 ...
 * Video data is captured every other line on lines 1, 3, 5, 7 ... 123, 125, 127
...
//      #endif
      ...
      if (cycles < LINE_DMA_COUNT) {
        if (capture_line) {
          ...
          int d_offset = BYTES_PER_ROW * raster + cycles;       
          byte old_byte = video_data[d_offset];
          //If any byte has changed save it and set flag to update display
          if (new_byte != old_byte) {
            changed = true;
            video_data[d_offset] = new_byte;
          } // if new_byte != old_byte                    
        } //if capture_line
...
void printVideoState () {
    ...
    printFlag(video_on);
    Serial.println(F("."));
    Serial.print(F("Init flag is "));
    printFlag(F(pixie_init));
    Serial.print(F(" and Clear flag is "));
    printFlag(F(pixie_clear));
    Serial.println(F("."));
  } //printVideoState
...
void isrPort1Output() {
  pixie_video = false;
  pixie_clear = !video_on;
  //pixie_clear = (new_byte != 0);
//  #if DEBUG
//    Serial.print(F("pixie_clear "));
//    printFlag(pixie_clear);
//    Serial.println();
//  #endif 
} //isrPort1Output

//Interrupt routine for Port 4 display
void isrPort4Output() {
   port4value = GPIOD_PDIR & BYTE_MASK;   
}
...
void setup(void) {
  ...
  //Set up Video Reset button pin 
  //Hardware SPI sets this pin as input for MISO, but it is unused by the display
  //We have an external pullup, to use it as an external Video Reset.
...
//      #endif
    ...
    if (pixie_video) {            
      startVideo(pixie_init);
      pixie_init = false;  //only initialize first time
//      #if DEBUG
//        printVideoState();
//      #endif             
    } else if (pixie_clear) {
      //Turn off display once
      stopVideo(pixie_clear);
      pixie_clear = false;   
    } else if( port4value != old_port4value ) {
      old_port4value = port4value;
     u8g2.firstPage();  
      do {
        drawDigit(old_port4value>>4,20);
        drawDigit(old_port4value&0x0F,80);
     } while( u8g2.nextPage() );
    } //if-else if-else
...
//    #endif
    ...
    pixie_video = false;
    //Next time video starts, initialize video display
    pixie_init  = true;
    //Always clear display for a reset
    stopVideo(true);
    setupVideo();
